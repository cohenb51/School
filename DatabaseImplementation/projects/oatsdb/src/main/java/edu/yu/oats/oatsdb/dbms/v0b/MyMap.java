...
public class MyMap<K, V> implements Map<K, V> { // create another map to house the object we need to update
	...
	private Object myGet(Object key) {
		...
		ReentrantLock lock = (ReentrantLock) originalMap.locks.get(key);
	...
	private V getLock(Object key, ReentrantLock lock) {
			logger.info("Getting lock " + lock );
			if (key == null) {
				return null;
			} // no locks on something not there
			if(lock == null) {
					ReentrantLock lock1 = new ReentrantLock();
					originalMap.locks.put(key, lock1);
					lock = lock1;
			}
			try {
				if (lock.isHeldByCurrentThread()) {
					logger.info("quick get");
					lock.lock();
				...
				for (int i = 0; i < 2; i++) {
					...
					logger.info("mysleeping");

					Thread.sleep(DBMSImpl.timeout);
					logger.info("mywokenup");

				}
				//lock.tryLock();
					if (!lock.isHeldByCurrentThread()) {
						logger.info("Failed to get key :(");
